...
def main():
    ...
    ap.add_argument("--max_steps", type=int, default=270)
    ...
    ap.add_argument("--eps_end", type=float, default=0.2)
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                ...
                with torch.no_grad():
                    ...
                    qs = qs.squeeze(0).cpu().numpy()
                a = int(np.argmax(qs))
            
            is_stuck = False
            if len(state_buffer) >= 3:
                if np.allclose(state_buffer[-1], state_buffer[-2]) and \
                np.allclose(state_buffer[-2], state_buffer[-3]):
                    is_stuck = True

            if is_stuck:
                a = np.random.choice([0,1,3,4])  # force rotate
            
            obs_curr = state_buffer[-1]

            # --- 1. STUCK (HIGHEST PRIORITY) ---
            if obs[17]: a=0
            # --- 2. IR ALIGNMENT (PUSHER) ---
            elif obs_curr[16] == 1:  # IR
                if np.random.rand() < 0.7:
                    a = 2  # FW
                else:
                    a = np.random.choice([1,3])  # small adjust

            # --- 3. DIRECTIONAL SEARCH ---
            left_signal = np.sum(obs_curr[0:4])
            right_signal = np.sum(obs_curr[4:8])

            if left_signal + right_signal > 0:  # only when something is detected
